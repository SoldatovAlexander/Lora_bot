...
Он обновляет систему, определяя тип видеокарты по ее DeviceID и устанавливая ядро Linux 6-й версии, 
которое нужно для A100/H100 серии на Ubuntu 22.04;

Устанавливает рекомендованные Ubuntu драйвера на вашу видеокарту;

Подключает репозитории Nvidia и устанавливает CUDA;

Прописывает необходимые переменные окружения для ее работы, попутно восстанавливая пути
 к /bin и /usr/bin, которые вы гарантированно поломаете, если будете следовать официальной
 инструкции;

Если у вас установлен Docker, то добавляет поддержку GPU в контейнеры.

Скрипт должен корректно отрабатывать на любых видеокартах старше 8xx версии и до последних 
моделей. Проверен на Tesla T4, GTX 1080, A4000, A5000, RTX 4090 и H100.

Сохраните его у себя в домашней директории как nvidia_install.sh, не забудьте задать через 
chmod +x nvidia_install.sh права на запуск и запустите через sudo nvidia_install.sh. 
Через 5-10 минут (в зависимости от скорости вашего интернета) вы получите работающую систему, 
на которой можно дальше экспериментировать с нейросетями.
